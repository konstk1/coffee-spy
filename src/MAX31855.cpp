...
static Logger log("SPI", Logger::Level::INFO);
...
MAX31855::MAX31855(int dataPin, int clkPin, int csPin) {
    log.Info("Initializing VSPI (MISO %d CLK %d CS %d)", dataPin, clkPin, csPin);
    ...
    status = spi_bus_add_device(VSPI_HOST, &devCfg, &m_spi);
    if (status != ESP_OK) {
        throw "Failed SPI BUS init";
    }
}
...
MAX31855::~MAX31855() {
    log.Verbose("Destorying MAX31855");
...
Either<int, MAX31855::Error> MAX31855::ReadTempC() const {
    ...
    log.Verbose("Data (0x%08X): TC %d F %d IC %d SCV %d SCG %d OC %d", rawData, (int)data->tcTemp, (int)data->fault, (int)data->internalTemp,
